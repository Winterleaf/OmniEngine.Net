...
namespace WinterLeaf.Engine.Classes.LiveScripts
    ...
    public class csFactory
        ...
        private bool ProcessXML(string filename)
            ...
            if (doc.Root.Name != "OverrideConfig") return false;

            data.XmlFileName = filename;

            if (doc.Root.HasAttributes)
                {
                foreach (var attr in doc.Root.Attributes().Where(attr => attr.Name == "Type"))
                    {
            ...
            if (data.ScriptType == xmlOverrideData.ScriptTypes.Undefined)
                ...
                #region
                lock (Omni.self.tick)
                    Omni.self.Error(">>csFactory::Error Processing C# Script XML File: '" + filename +
                                    "' Reason: Unknown ScriptType.");
                return false;
                #endregion
                }
            ...
            if (data.ReferencedAssemblies.Count == 0)
                ...
                #region
                lock (Omni.self.tick)
                    Omni.self.Error(">>csFactory::WARNING: Script XML File: '" + filename +
                                    "' Reason: No Referenced Assemblies.");
                #endregion
                }
            ...
            if (data.FunctionOverrides.Count == 0 && data.ScriptType != xmlOverrideData.ScriptTypes.Classes)
                ...
                lock (Omni.self.tick)
                    Omni.self.Error(">>csFactory::WARNING: Script XML File: '" + filename +
                                    "' Reason: No Function Overrides Defined.");
                #endregion
                }
            #region
//#if DEBUG
...
//#endif
            ...
            #region
            bool haderror = false;
            foreach (var f in data.CsFileNames)
                {
                if (!File.Exists(f))
                    lock (Omni.self.tick)
                        {
                        Omni.self.Error(">>csFactory::Error Processing C# Script XML File: '" + filename +
                                        "' C# Script file '" + f + "' does not exist.");
                        haderror = true;
                        }
                }
        ...
        private void StartWatchers()
            ...
            _fswatcher.NotifyFilter = NotifyFilters.Attributes | NotifyFilters.CreationTime |
        ...
        private void UnRegisterScriptXml(string xmlfilename)
            ...
            if (!_overrideXmlFilenameIndex.ContainsKey(xmlfilename)) return;
        ...
        private xmlOverrideData getObjectByCsFilePath(string csfilename)
            {
            return
                _overrideData.Values.FirstOrDefault(
                    t => t.CsFileNames.Any(f => String.Equals(f, csfilename, StringComparison.CurrentCultureIgnoreCase)));
            }
        ...
        internal List<xmlOverrideData> GetMemberOverrides(string namespc)
            {
            return
                _overrideData.Values.Where(
                    od =>
                        String.Equals(od.NameSpace, namespc, StringComparison.CurrentCultureIgnoreCase) &&
                        od.ScriptType == xmlOverrideData.ScriptTypes.MemberFunction).ToList();
            }
