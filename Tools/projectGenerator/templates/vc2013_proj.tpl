...
<Project DefaultTargets="Build" ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  ...
  <ItemDefinitionGroup Condition="'$(Configuration)|$(Platform)'=='Debug|Win32'">
    ...
    <ClCompile>
      ...
      <BasicRuntimeChecks>{$projBasicRuntimeChecks}</BasicRuntimeChecks>
      <StringPooling>true</StringPooling>
      <RuntimeLibrary>{if $projRuntimeDebug == 1}MultiThreadedDebug{else}MultiThreadedDebugDLL{/if}</RuntimeLibrary>
      
	  <BufferSecurityCheck>{if $projBufferSecurityCheck == 0}false{else}true{/if}</BufferSecurityCheck>
	  
      <FunctionLevelLinking>{if $projFunctionLevelLinking == 0}false{else}true{/if}</FunctionLevelLinking>
