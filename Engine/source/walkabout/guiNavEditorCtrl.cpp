...
IMPLEMENT_CALLBACK(GuiNavEditorCtrl, onLinkDeselected, void, (), (), "");
IMPLEMENT_CALLBACK(GuiNavEditorCtrl, onPlayerDeselected, void, (), (), "");
IMPLEMENT_CALLBACK(GuiNavEditorCtrl, onPlayerSelected, void, (S32 linkDataFlags), (linkDataFlags), "");
IMPLEMENT_CALLBACK(GuiNavEditorCtrl, onLinkSelected, void, (S32 linkDataFlags), (linkDataFlags), "");
IMPLEMENT_CALLBACK(GuiNavEditorCtrl, onModeSet, void, (String mode), (mode), "");
IMPLEMENT_CALLBACK(GuiNavEditorCtrl, paletteSync, void, (String mode), (mode), "");
...
void GuiNavEditorCtrl::spawnPlayer(const Point3F &pos)
{
   ...
   if(obj)
   {
	  ...
	  onPlayerSelected_callback(mPlayer->mLinkTypes.getFlags());
#else
	  onPlayerSelected_callback(0);
#endif
   }
}
   ...
   if(mMode == mLinkMode && !mMesh.isNull())
   {
      if(gServerContainer.castRay(startPnt, endPnt, StaticObjectType, &ri))
      {
         ...
         if(link != -1)
         {
			...
			onLinkSelected_callback(d.getFlags());
         }
         else
         {
            if(mLink != -1)
...
DefineConsoleMethod(GuiNavEditorCtrl, getMode, const char*, (), , "")
{
   return object->getMode();
}

//ConsoleMethod(GuiNavEditorCtrl, setMode, void, 3, 3, "setMode(String mode)")
DefineConsoleMethod(GuiNavEditorCtrl, setMode, void, (String newMode) , , "setMode(String mode)")
{
